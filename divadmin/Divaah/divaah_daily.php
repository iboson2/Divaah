...
<body class="fixed-leftside fixed-header">
	...
	<div class="wrapper">
				...
				<div class="title">Navigation</div>
					<ul class="nav-sidebar">
                        ...
                        <li class="nav-dropdown active open">
                            ...
                            <ul>
								<li class="active"><a href="divaah_daily.php">Daily Wears</a></li>
        ...
        <div class="rightside bg-grey-100">
			...
			<br><br>
            <div class="page-head">
				<h1 class="page-title">Daily Wears<small>Update all page details here.</small></h1>
				<!-- BEGIN BREADCRUMB -->
				<ol class="breadcrumb">
					<li><a href="index.php"><i class="ion-home margin-right-5"></i> Dashboard</a></li>
					<li class="active">Daily Wears</li>
				</ol>
				<!-- END BREADCRUMB -->
			</div>
			<!-- END PAGE HEADING -->
...
if(isset($_FILES['uploaded_file'])) {
    ...
    if($_FILES['uploaded_file']['error'] == 0) {
        ...
        if($result){
            header("Location: divaah_daily.php");
        }
        else {
...
?>					
							...
							<div class="panel-body">
								<h3 class="color-grey-700 margin-top-10">Banner image</h3>
								<p class="text-light margin-bottom-30">Drag’n’drop the banner image.</p>								
								<?php  echo"<img  alt='' style='width:100%;' src='data:image/jpeg;base64,$log' >";?>
								<form  action="divaah_daily.php" method="post" enctype="multipart/form-data" ><br>								
	...
	<?php
	    ...
	    if (!empty($product_array)) { 
	    foreach($product_array as $key=>$value){
			$ie=$product_array[$key]["ID"];
	    $bosd=$product_array[$key]["Name"];
        $logt=base64_encode($product_array[$key]['img']);	 
	     }
	     }
...
if(isset($_FILES['uploaded_file'])) {
    ...
    if($_FILES['uploaded_file']['error'] == 0) {
        ...
        $query =  "UPDATE image SET img='$data' WHERE ID='".$ie."'";
        // Execute the query
        $result = $dbLink->query($query);
...
?>									
							...
							<div class="panel-body">
								<h3 class="color-grey-700 margin-top-10">Sub Category Image (Kurti)</h3>
								<p class="text-light margin-bottom-30">Drag’n’drop the Sub Category Image (Kurti).</p>
								<?php  echo"<img  alt='' style='width:25%;' src='data:image/jpeg;base64,$logt' >";?>
								<form  action="divaah_daily.php" method="post" enctype="multipart/form-data" ><br>								
									<input type="file" name="uploaded_file" value="Change Image" ><br>
	...
	<?php
	    ...
	    foreach($product_array as $key=>$value){
			$ie=$product_array[$key]["ID"];
	    $bosd=$product_array[$key]["Name"];
        $logy=base64_encode($product_array[$key]['img']);	 
	     }
	     }
...
?>								
							...
							<div class="panel-body">
								<h3 class="color-grey-700 margin-top-10">Sub Category Image (Sarees)</h3>
								<p class="text-light margin-bottom-30">Drag’n’drop the Sub Category Image (Sarees).</p>
								<?php  echo"<img  alt='' style='width:25%;' src='data:image/jpeg;base64,$logy' >";?>
								<form  action="divaah_daily.php" method="post" enctype="multipart/form-data" ><br>								
									<input type="file" name="uploaded_file" value="Change Image" ><br>
	    ...
	    foreach($product_array as $key=>$value){
			$ie=$product_array[$key]["ID"];
	    $bosd=$product_array[$key]["Name"];
        $logu=base64_encode($product_array[$key]['img']);	 
	     }
	     }
...
?>							
							...
							<div class="panel-body">
								<h3 class="color-grey-700 margin-top-10">Sub Category Image (Salwar Materials)</h3>
								<p class="text-light margin-bottom-30">Drag’n’drop the Sub Category Image (Salwar Materials).</p>
								<?php  echo"<img  alt='' style='width:25%;' src='data:image/jpeg;base64,$logu' >";?>
								<form  action="divaah_daily.php" method="post" enctype="multipart/form-data" ><br>								
									<input type="file" name="uploaded_file" value="Change Image" ><br>
