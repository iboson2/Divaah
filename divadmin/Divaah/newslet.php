					...
					<div class="col-lg-12"> 
						<div class="panel">
							<div class="panel-title no-border bg-white">
								<div class="panel-head"><i class="ion-arrow-graph-up-right"></i>Newsletter</div>
...
if(isset($_GET['remb'])){
		...
		$product = $db_handle->runQuery("DELETE FROM email  WHERE ID = '".$idf."' and email = '".$idg."'");
...
?>													
							...
							<?php
		$product_array = $db_handle->runQuery("SELECT * FROM email");
	    if (!empty($product_array)) { 
	    foreach($product_array as $key=>$value){
	    $id=$product_array[$key]["ID"];
        $en=$product_array[$key]["email"];	
		...
		?>			
										<tr>
										<td style=" border:1px solid #000; text-align:center;"><?=$en ?></td>
																			
										<form action="newslet.php" method="get">
										<input type="hidden" name="ids" value="<?= $id ?>">
										<input type="hidden" name="em" value="<?= $en ?>">
