...
<style>.snip1585{background-color:#000;color:#fff;display:inline-block;font-family:'Roboto',sans-serif;font-size:24px;margin:8px;max-width:362px;min-width:250px;overflow:hidden;position:relative;text-align:center;width:100%}.snip1585 *{-webkit-box-sizing:border-box;box-sizing:border-box;-webkit-transition:all .45s ease;transition:all .45s ease}.snip1585:before,.snip1585:after{background-color:rgba(0,0,0,0.5);border-top:50px solid rgba(0,0,0,0.5);border-bottom:50px solid rgba(0,0,0,0.5);position:absolute;top:0;bottom:0;left:0;right:0;content:'';-webkit-transition:all .3s ease;transition:all .3s ease;z-index:1;opacity:0}.snip1585:before{-webkit-transform:scaleY(2);transform:scaleY(2)}.snip1585:after{-webkit-transform:scaleY(2);transform:scaleY(2)}.snip1585 img{vertical-align:top;max-width:100%;backface-visibility:hidden}.snip1585 figcaption{position:absolute;top:0;bottom:0;left:0;right:0;align-items:center;z-index:1;display:flex;flex-direction:column;justify-content:center;line-height:1.1em;opacity:1;z-index:2;-webkit-transition-delay:0s;transition-delay:0s}.snip1585 h3{font-size:40px;font-weight:400;letter-spacing:1px;margin:0;text-transform:uppercase}.snip1585 h3 span{display:block;font-weight:700}.snip1585 a{position:absolute;top:0;bottom:0;left:0;right:0;z-index:3}.snip1585:hover>img,.snip1585.hover>img{opacity:.9}.snip1585:hover:before,.snip1585.hover:before,.snip1585:hover:after,.snip1585.hover:after{-webkit-transform:scale(1);transform:scale(1);opacity:1}.snip1585:hover figcaption,.snip1585.hover figcaption{opacity:1;-webkit-transition-delay:.1s;transition-delay:.1s}</style>
</head>
<body>
...
<div><a id=btnEmpty href="party.php?action=empty" href=javascript:; class=simpleCart_empty onclick=resetForm(this.form)>
Empty Cart
</a></div>
<?php } ?>
...
<li class=clearfix>
<?php echo"<img class=cart_item_product src=data:image/jpeg;base64,$roc>"; ?>
<a href=partysalwars.php class=cart_item_title><?php echo $item["name"]; ?></a>
<span class=cart_item_price><?php echo $item["quantity"]; ?> × &nbsp;<i class="fa fa-inr"></i>&nbsp;
<?php echo $item["price"]; ?></span>
</li>
</ul>
...
<div class=clearfix><span class=cart_subtotal>bag subtotal: <b>&nbsp;<i class="fa fa-inr"></i>&nbsp; <?=$prd ?></b></span></div>
<div>
<a id=btnEmpty href="party.php?go=empty" href=javascript:; class=simpleCart_empty onclick=resetForm(this.form)>
Empty Cart
</a></div>
...
<?php $product_array=$db_handle->runQuery("SELECT * FROM image where ID = '18'");
if (!empty($product_array)) {
foreach($product_array as $key=>$value){
...
<a class="banner nobord" href=javascript:void(0)><?php echo"<img alt src=data:image/jpeg;base64,$log>";?></a>
</div>
<?php } } ?>
</div>
<div class="col-lg-12 col-sm-12 col-sm-12 padbot50">
<div class=container align=center style="padding:15px 15px">
<div class=row>
<?php $product_array=$db_handle->runQuery("SELECT * FROM image where ID = '10'");
if (!empty($product_array)) {
foreach($product_array as $key=>$value){
$bos=$product_array[$key]["Name"];
$log=base64_encode($product_array[$key]['img']);
?>
<figure class=snip1585 style=background-color:transparent>
<?php echo"<img alt=sample70 src=data:image/jpeg;base64,$log>";?>
<figcaption>
<h3 style=color:#f9af46><?=$bos ?><span></span></h3>
</figcaption>
<a href=partysalwars.php></a>
</figure>
<?php } } ?>
<?php $product_array=$db_handle->runQuery("SELECT * FROM image where ID = '11'");
if (!empty($product_array)) {
foreach($product_array as $key=>$value){
$bos=$product_array[$key]["Name"];
$log=base64_encode($product_array[$key]['img']);
